...
class VggtPerspectiveTransform(LightningModule):
    ...
    def _shared_step(self, batch: list[SceneSample], stage: str) -> dict[str, th.Tensor]:
        ...
        target_centers = self._camera_centers(projected.pose)
        centers_target_rel = target_centers - target_centers[:1]

        depth_gt = projected.depth
        alpha = projected.alpha
        depth_loss = self._compute_depth_loss(depth_gt, depth_faces, alpha)

        if views > 1:
            rot_loss = self._geodesic_so3(target_rot[1:], rotation_rel[1:]).mean()
            trans_loss = ((centers_target_rel[1:] - centers_pred_rel[1:]) ** 2).mean()
        else:
            zero = depth_loss.new_zeros(())
            rot_loss = zero
            trans_loss = zero
